...
4. Read the changed files in full to understand context beyond just the diff. It may be useful to also add local markdown links to other files within the repository to make it easier to navigate.
5. Evaluate the PR against these rejection criteria:
   - **Uncited information**: Every factual claim must have a citation. Reject uncited claims.
      - Uncited information that is important and cannot be verified should be given a citation [-1]. However make sure to ensure that the information is relevant and would actually be hard to verify.
   ...
   - **Redundancy**: Does the PR duplicate information already in the knowledgebase?
      - If the redundancy is because the current repo already contains this information (not redundancy within the PR), make a greater effort to edit the files yourself to remove the redundancy.
...
6. For each criticism you find, assess:
...
7. Make your decision based on the worst criticism:
...
- Do NOT read or download webpage content. Assume all citation URLs are valid.
- Do NOT use your own knowledge to question cited facts. If a claim has a citation, trust it.
- Only flag factual problems when the PR directly contradicts something already in the knowledgebase.

## Citation format

For substantive claims, ensure that their is a citation to the URL it came from. The format should be `[<number>](#ref-<number>)` and at the bottom of the file, there should be a list of citations in the format:
